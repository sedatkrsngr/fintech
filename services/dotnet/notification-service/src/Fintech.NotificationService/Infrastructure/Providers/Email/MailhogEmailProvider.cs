...
public sealed class MailhogEmailProvider : IEmailProvider
{
    private readonly MailhogOptions _options;

    public MailhogEmailProvider(IOptions<MailhogOptions> options)
    {
    ...
    public bool CanHandle(string providerKey, NotificationProviderType providerType)
    {
        return providerType == NotificationProviderType.Smtp
            && string.Equals(providerKey, "mailhog", StringComparison.OrdinalIgnoreCase);
    }
